/*
 This is a Swift conversion of SERD by David Robillard.
 
 This effort is done as an exercise in understanding RDF files for iOS and macOS by Michael Rowe.
 
 The updated work is copywrited by Michael Rowe 2022, based on the rights granted below by David Robilard
...
{
	switch (status) {
	case SWIFT_SERD_SUCCESS:        return (const uint8_t*)"Success";
	case SWIFT_SERD_FAILURE:        return (const uint8_t*)"Non-fatal failure";
	case SWIFT_SERD_ERR_UNKNOWN:    return (const uint8_t*)"Unknown error";
	case SWIFT_SERD_ERR_BAD_SYNTAX: return (const uint8_t*)"Invalid syntax";
	case SWIFT_SERD_ERR_BAD_ARG:    return (const uint8_t*)"Invalid argument";
	case SWIFT_SERD_ERR_NOT_FOUND:  return (const uint8_t*)"Not found";
	case SWIFT_SERD_ERR_ID_CLASH:   return (const uint8_t*)"Blank node ID clash";
	case SWIFT_SERD_ERR_BAD_CURIE:  return (const uint8_t*)"Invalid CURIE";
	case SWIFT_SERD_ERR_INTERNAL:   return (const uint8_t*)"Internal error";
	}
...
serd_update_flags(const uint8_t c, SerdNodeFlags* const flags)
{
	switch (c) {
	case '\r': case '\n':
		*flags |= SWIFT_SERD_HAS_NEWLINE;
		break;
	case '"':
		*flags |= SWIFT_SERD_HAS_QUOTE;
	}
}
